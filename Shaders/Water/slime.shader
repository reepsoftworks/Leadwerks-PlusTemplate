...
void main(void)
{
	...
	vec2 movement = ex_texcoords0;
	movement.x -= time * speed;
	movement.y -= time * speed;

	vec4 outcolor = ex_color;
	vec3 normal = texture(texture1,movement).xyz * 2.0 - 1.0;
	float ao = normal.z;
	normal = ex_tangent*normal.x + ex_binormal*normal.y + ex_normal*normal.z;
	normal=normalize(normal);

	//Modulate blend with diffuse map
	outcolor *= texture(texture0,movement);
